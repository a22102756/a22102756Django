...
from .models import Post, Lingua, Competencia, Professor, Disciplina, Pessoa, Tarefa, Comment, Noticia, Projects, \
    Author, Advisor, Lab
...
def add_to_publication(request, post_id):
    ...
    if request.method == 'POST':
        ...
        # Adicione qualquer processamento ou validação adicional, se necessário

    return render(request, 'portfolio/pubs.html')
...
def authors(request):
    ...
    return render(request, 'portfolio/projetos.html', {authors})
...
def search_praia(request):
    praia = request.GET.get('praia')
    if praia:
        regioes = Regiao.objects.filter(praia__regiao__in=praia)
    else:
        regioes = Regiao.objects.all()
    return render(request, 'portfolio/verao.html', {'regioes': regioes, 'praia':praia})
...
def add_comment(request, post_id):
    if request.method == 'POST':
        ...
        return redirect('portfolio:publication', post_id=post_id)
    else:
        return redirect('portfolio:publication')
...
def author_view(request, author_id):
    ...
    projects = Projects.objects.filter(authors=author)
    return render(request, 'portfolio/projetos.html', {'author': author, 'projects': projects})


def advisor_view(request, advisor_id):
    advisor = get_object_or_404(Advisor, pk=advisor_id)
    projects = Projects.objects.filter(advisors=advisor)
    return render(request, 'advisor.html', {'advisor': advisor, 'projects': projects})
